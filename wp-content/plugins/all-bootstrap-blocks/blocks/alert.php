...
function areoi_render_block_alert( $attributes, $content ) 
{
	...
	$icon = !empty( $attributes['icon'] ) ? '<img class="icon bi flex-shrink-0 me-2" src="' . esc_attr( $attributes['icon']['url'] ) . '" width="' . esc_attr( $attributes['icon']['width'] ) . '" height="' . esc_attr( $attributes['icon']['height'] ) . '" alt="' . esc_attr( $attributes['icon']['alt'] ) . '">' : '';
	$output = '
